...
document.addEventListener('DOMContentLoaded', function() {
    const goalInfoBox = document.getElementById('goalInfoBox');
    
    const goalData = {
        username: "말하는 감자",
        title: "주 3회 요가",
        frequency: "일주일에 3회",
        duration: "1회당 1시간 30분 소요",
        image: "asset/Logo2 1.svg",
        category: "운동",
        cartIcon: "asset/Basket_alt_3.svg"
    };
    
    goalInfoBox.innerHTML = `
    <div class="goal-header">
        <img src="${goalData.image}" alt="Title Logo" class="title-logo">
        <div class="goal-title">
            <div class="username">${goalData.username}</div>
            <div>${goalData.title}</div>
            <div class="goal-category">
                <button class="category-button">${goalData.category}</button>
                <img src="${goalData.cartIcon}" alt="Cart Icon" class="cart-icon">
            </div>
    <hr class="goal-separator">
    <div class="goal-details">
        <div>${goalData.frequency}</div>
        <div>${goalData.duration}</div>
    </div>
`;
    ...
    document.getElementById('reviewForm').addEventListener('submit', function(e) {
        ...
        if (reviewInput) {
            ...
            newReview.innerHTML = `
                ...
                <div class="review-content">${reviewInput}</div>
